...
export interface ReputationScore {
  ...
  reputationLevel: 'poor' | 'fair' | 'good' | 'excellent';
  requirements: {
    [key: string]: {
      minScore: number;
      eligible: boolean;
...
export class ReputationCalculator {
  ...
  private static readonly REPUTATION_LEVELS = {
   ...
   */
  static async calculateReputationScore(agentDid: string): Promise<ReputationScore> {
    try {
      ...
      const finalScore = Math.round(
        successScore * this.SCORE_WEIGHTS.transactionSuccess +
  ...
  private static calculateMetrics(transactions: any[], accountCreatedAt: Date) {
  ...
  private static calculateSuccessScore(transactions: any[]): number {
  ...
  private static calculateVolumeScore(transactions: any[]): number {
    if (transactions.length === 0) return 0;
  ...
  private static calculateAgeScore(accountCreatedAt: Date): number {
  ...
  private static calculateFrequencyScore(transactions: any[]): number {
    if (transactions.length === 0) return 0;
  ...
  private static calculateDiversityScore(transactions: any[]): number {
    if (transactions.length === 0) return 0;
  ...
  private static calculateRecentActivityScore(transactions: any[]): number {
    if (transactions.length === 0) return 0;
   ...
   */
  static async getReputationScore(agentDid: string, useCache: boolean = true): Promise<ReputationScore> {
