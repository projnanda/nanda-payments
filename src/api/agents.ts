...
agents.post("/agents/resolve", async (req, res) => {
  ...
  const doc = await AgentModel.findOneAndUpdate(
    { did: body.did },
    { $set: update, $setOnInsert: { did: body.did } },
...
agents.get("/agents", async (req, res) => {
  const { did, q, limit = "50", after } = req.query as any;
  const filter: any = {};
...
agents.get("/agents/:did", async (req, res) => {
  ...
  if (!doc) return res.status(404).json({ error: { code: "NOT_FOUND", message: "Agent not found" } });
  res.json(doc);
});
...
agents.delete("/agents/:did", async (req, res) => {
  ...
  if (del.deletedCount === 0) return res.status(404).json({ error: { code: "NOT_FOUND", message: "Agent not found" } });
