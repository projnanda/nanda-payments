...
function parseWalletProof(proof: string) {
...
wallets.post("/wallets", async (req, res) => {
  ...
  if (!agent) return res.status(404).json({ error: { code: "AGENT_NOT_FOUND", message: "Agent not found" } });
...
wallets.post("/wallets/:walletId/attach", async (req, res) => {
  try {
    ...
    if (!wallet) return res.status(404).json({ error: { code: "WALLET_NOT_FOUND", message: "Wallet not found" } });
...
wallets.post("/wallets/:walletId/detach", async (req, res) => {
  try {
    ...
    const updated = await AgentModel.findOneAndUpdate(
      { did },
      { $unset: { "payments.np.walletId": "" }, $set: { "payments.np.walletProofVerified": false } },
      { new: true }
    );
    return res.json({ ok: true, agent: updated });
  } catch (e: any) {
...
wallets.post("/wallets/link", async (req, res) => {
  try {
    ...
    await WalletLinkNonceModel.findOneAndUpdate(
      { did },
      { $set: { nonce }, $setOnInsert: { did } },
...
wallets.post("/wallets/link/verify", async (req, res) => {
  try {
    ...
    if (primaryFactsUrl && primaryFactsUrl !== agent.primaryFactsUrl) {
      await AgentModel.updateOne({ did }, { $set: { primaryFactsUrl } });
      agent.primaryFactsUrl = primaryFactsUrl as any;
    }

    const nonceDoc = await WalletLinkNonceModel.findOne({ did });
    if (!nonceDoc) return res.status(422).json({ error: { code: "LINK_NOT_REQUESTED", message: "Start link with /wallets/link first" } });
    ...
    if (!proofStr || !walletId) {
      if (!agent.primaryFactsUrl) return res.status(400).json({ error: { code: "FACTS_URL_MISSING", message: "Agent.primaryFactsUrl is not set" } });
      const resp = await fetch(agent.primaryFactsUrl);
      if (!resp.ok) return res.status(400).json({ error: { code: "FACTS_FETCH_FAILED", message: `Failed to fetch Facts: ${resp.status}` } });
      ...
      if (!proofStr) return res.status(422).json({ error: { code: "PROOF_NOT_FOUND", message: "walletProof not present in Facts" } });
    }

    const parsed = parseWalletProof(proofStr);
    if (!parsed.nonce || !parsed.walletId) return res.status(400).json({ error: { code: "PROOF_INVALID", message: "walletProof must include nonce and walletId" } });

    if (parsed.nonce !== nonceDoc.nonce) return res.status(409).json({ error: { code: "NONCE_MISMATCH", message: "Provided proof nonce does not match" } });
    if (providedWalletId && providedWalletId !== parsed.walletId) return res.status(409).json({ error: { code: "WALLET_ID_MISMATCH", message: "walletId mismatch" } });

    // Verify the wallet exists and belongs to the DID
    const wallet = await WalletModel.findById(parsed.walletId);
    if (!wallet) return res.status(404).json({ error: { code: "WALLET_NOT_FOUND", message: "Wallet not found" } });
    if (wallet.agentDid !== did) return res.status(403).json({ error: { code: "WALLET_OWNERSHIP_ERROR", message: "Wallet does not belong to DID" } });
