import { WalletModel, TransactionModel, EventModel, IdemModel, NP_CURRENCY, NP_SCALE } from "../models/index.js";
...
export async function createTransaction(opts) {
    ...
    if (dst && dst.status !== "active")
        throw new Error("dest wallet not active");
    ...
    if (type === "mint") {
        if (!dst)
            throw new Error("destWalletId required for mint");
        ...
        if (!src)
            throw new Error("sourceWalletId required for burn");
    ...
    const snapshots = [];
    if (src) {
        const updated = await WalletModel.findOneAndUpdate({ _id: src._id, status: "active", balance: { $gte: (src.limits?.allowOverdraft ?? false) ? -1e15 : amountValue } }, { $inc: { balance: -amountValue, seq: 1 } }, { new: true });
        if (!updated) {
            const err = new Error("insufficient funds or wallet not active");
            err.httpCode = 402;
            throw err;
        }
    ...
    if (dst) {
        const updated = await WalletModel.findOneAndUpdate({ _id: dst._id, status: "active" }, { $inc: { balance: amountValue, seq: 1 } }, { new: true });
        if (!updated)
            throw new Error("dest wallet update failed");
        snapshots.push({ walletId: updated._id.toString(), balanceAfter: updated.balance, seq: updated.seq });
    }
    // write transaction
...
export async function createTransactionWithReputation(opts) {
    const { type, sourceWalletId, destWalletId, amountValue, reasonCode, idempotencyKey, actor, facts, metadata, reputationHash, skipReputationCheck } = opts;
    if (amountValue <= 0)
        throw new Error("amount must be positive");
    const existing = await IdemModel.findOne({ key: idempotencyKey });
    if (existing) {
    ...
    if (actor?.type === "agent" && actor.did && !skipReputationCheck) {
        if (!reputationService || !serviceReady) {
            const error = new Error("Reputation service not initialized. Please call /reputation/initialize first.");
        ...
        try {
            ...
            const reputationResult = await reputationService.verifyTransactionReputation({ reputationHash }, actor.did, minRequiredScore);
            ...
            if (!reputationResult.isValid) {
                const error = new Error(`Reputation verification failed: ${reputationResult.error}`);
    ...
    if (reputationService && actor?.type === "agent" && actor.did && !skipReputationCheck) {
        try {
            ...
            await reputationService.updateReputationAfterTransaction(tx._id.toString(), actor.did, transactionSuccessful, currentScore);
        }
