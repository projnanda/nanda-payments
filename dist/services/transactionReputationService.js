...
export class TransactionReputationService {
     ...
     */
    async verifyTransactionReputation(transactionData, agentDid, minRequiredScore = 50) {
        try {
            ...
            const reputationScore = await this.verifier.decryptReputationHash(transactionData.reputationHash, agentDid);
            if (!reputationScore) {
                return {
                    isValid: false,
     ...
     */
    async updateReputationAfterTransaction(transactionId, agentDid, wasSuccessful, currentScore) {
        try {
     ...
     */
    validateReputationHashFormat(reputationHash) {
        try {
     ...
     */
    getMinimumReputationRequirement(transactionType, transactionAmount) {
        ...
        return requirements[transactionType] || 50; // Default to 50
    }
    /**
     ...
     */
    static generateSampleReputationHash(agentDid, score, verifierPublicKey) {
