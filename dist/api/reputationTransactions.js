...
reputationTransactions.get("/reputation/requirements", async (req, res) => {
    const { transactionType, amount } = req.query;
    if (!transactionType) {
    ...
    const minScore = requirements[transactionType] || 50;
    res.json({
        transactionType,
        amount: amountValue,
...
reputationTransactions.post("/agents/:did/reputation/verify", async (req, res) => {
    try {
        ...
        const verificationResult = await reputationService.verifyTransactionReputation({ reputationHash }, did, 0 // We don't need a minimum score for verification, just validation
        );
